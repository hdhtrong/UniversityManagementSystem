...
namespace NotificationService.API.Extentions
{
    public static class ConsulExtensions
    {
        public static async Task RegisterWithConsulAsync(this WebApplication app, IConfiguration configuration)
        {
            ...
            var serviceName = configuration["Consul:ServiceName"] ?? "notification-service";
            var servicePort = int.Parse(configuration["Consul:ServicePort"] ?? "7009");
