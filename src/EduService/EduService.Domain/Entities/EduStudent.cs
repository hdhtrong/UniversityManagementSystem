...
namespace EduService.Domain.Entities
{
    ...
    public class EduStudent : AuditableEntity
    {
        public EduStudent()
        {
            ID = Guid.NewGuid();
        ...
        public EduStudent(string studentId)
        {
            ID = Guid.NewGuid();
            StudentID = studentId;
        }
        ...
        [ForeignKey("ClassID")]
        public EduClass? Class { get; set; } // Lớp (liên kết)

        public ICollection<EduEnrollment>? Enrollments { get; set; }
        public ICollection<EduTuitionFee>? TuitionFees { get; set; }
    }
}
